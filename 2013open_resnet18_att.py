...
from receiptData import get2013openData
import time
import warnings
...
def validate(val_loader, model, criterion, f):
    ...
    df.to_csv("2013open_resnet18_att"+".csv", index=False,mode='w')  # 路径可以根据需要更改
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        data.to_csv('2013open_resnet18_att.csv', mode='a', header=False, index=False)
...
def fit_and_score(params):
    ...
    for epoch in range(epochs):
        ...
        if val_epoch_accuracy.avg > best_val_acc:
            ...
            }, True, "2013open_resnet_att_fold" + str(params['f']) + '_val')
    ...
    if best_score > loss.avg:
        ...
        save_checkpoint({
            ...
            'best_prec1': acc.avg,
            'optimizer': best_optimizer.state_dict(),
        }, True, "2013open_resnet_att_fold"+str(params['f'])+'_loss')
...
if __name__ == '__main__':
    ...
    space = {
        ...
        'batch_size': hp.choice('batch_size', [3, 5, 7, 9]),
        'learning_rate': hp.loguniform("learning_rate", np.log(0.00001), np.log(0.01)),
        'f': 0}
    outfile = open("2013open_resent18_att" + '.log', 'w')
    ...
    for f in range(1, 3):
        ...
        outfile.write("\nResnet18 no attention Hyperopt trials")
