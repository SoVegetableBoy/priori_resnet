...
def validate(val_loader, model, criterion, file_path):
    ...
    df = pd.DataFrame(columns=['time', 'Epoch', 'Loss', 'acc','prec','rec','f1','target','prediction'])  # 列名
...
if __name__ == '__main__':
    ...
    outfile = open("2013closed_resent_att_load" + '.log', 'w')
    ...
    for f in range(3):
        files_path = modelUtil().readModel('./checkpoints', '2013closed_resnet_att_fold'+str(f), 'pth.tar')
